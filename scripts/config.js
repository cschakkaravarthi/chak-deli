...
async function vaultGetSecret(token, secret) {
  return new Promise(async (res, rej) => {
    try {
      let resp = await axios.get(vaultAddr + '/v1/umg-central/data/' + secret, {
        headers: {
          'X-Vault-Token': token
        }
      })
...
(async () => {
  try {
    ...
    let data = await vaultGetSecret(token, 'umgc_delivery/' + process.env.NODE_ENV)
    if(process.env.API_URL) { 
      data.centralApi.baseUrl = process.env.API_URL;
    }
    let configData = JSON.stringify(data);
    let tpl = `window.env = ${configData}`;

    fs.writeFileSync("src/config/index." + process.env.NODE_ENV + ".js", tpl);
